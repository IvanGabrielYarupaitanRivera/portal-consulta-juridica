<script lang="ts">
	...
	let {
		...
		selectedConsulta,
		showResponderConsultaModal = $bindable(false)
	} = $props();
...
{#if showResponderConsultaModal}
	...
	>
		<dialog
			...
			aria-labelledby="respond-consulta-modal-title"
			aria-describedby="respond-consulta-modal-description"
			onclick={(e) => e.stopPropagation()}
		>
			<form
				method="POST"
				action="/privado/actions/consultas?/responderConsulta"
				use:enhance={handleResponderConsulta}
				...
				<header class="border-b border-gray-200 bg-white p-4 lg:p-6">
					<div class="flex items-start justify-between gap-4">
						<hgroup class="min-w-0 flex-1">
							<div class="flex items-center gap-3">
								...
								<div class="min-w-0 flex-1">
									<h2
										id="respond-consulta-modal-title"
										class="text-lg font-semibold text-gray-900 lg:text-xl"
									>
										Responder consulta
									</h2>
									<p
										id="respond-consulta-modal-description"
										class="mt-0.5 text-sm text-gray-500 lg:text-base"
									>
										Revisa la consulta y escribe la respuesta. Esta acción marcará la consulta como
										respondida.
									</p>
								</div>
							</div>
						...
						<button
							...
							aria-label="Cerrar modal de responder consulta"
						>
					...
					<div class="h-full max-h-[60dvh] space-y-6 overflow-y-auto px-1">
						<section class="space-y-4">
							<fieldset class="space-y-4">
								...
								<div class="space-y-1.5">
									<label
										for="respond-consulta-respuesta"
										class="block text-sm font-medium text-gray-700"
									>
										Respuesta
									</label>
									<textarea
										id="respond-consulta-respuesta"
										name="respuesta"
										rows="6"
										required
										placeholder="Escribe la respuesta para el consultante..."
										class="w-full resize-y rounded-lg border border-gray-200 px-3 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 focus:ring-2 focus:ring-yellow-500 focus:outline-none"
										>{selectedConsulta?.respuesta ?? ''}</textarea
									>
								</div>
							</fieldset>
						</section>
				...
				<footer class="border-t border-gray-200 bg-gray-50 p-4 lg:p-6">
					<div class="flex flex-col gap-3 lg:flex-row lg:justify-end">
						...
						<button
							...
							aria-label="Enviar respuesta a la consulta"
						>
							Enviar respuesta
						</button>
					</div>
